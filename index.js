...
async function startMenu(menu) {
    ...
    switch (mode) {
        case "web":
            ...
            break
        case "zksync":
            await zkSyncFetchDataAndPrintTable().catch(error => { console.error('Виникла помилка:', error)})
            if (startOver) await startMenu()
            break
        case "starknet":
            await starknetFetchDataAndPrintTable().catch(error => { console.error('Виникла помилка:', error)})
            if (startOver) await startMenu()
            break
        case "layerzero":
            await layerzeroFetchDataAndPrintTable().catch(error => { console.error('Виникла помилка:', error)})
            if (startOver) await startMenu()
            break
        case "zora":
            await zoraFetchDataAndPrintTable().catch(error => { console.error('Виникла помилка:', error)})
            if (startOver) await startMenu()
            break
        case "base":
            await baseFetchDataAndPrintTable().catch(error => { console.error('Виникла помилка:', error)})
            if (startOver) await startMenu()
            break
        case "aptos":
            await aptosFetchDataAndPrintTable().catch(error => { console.error('Виникла помилка:', error)})
            if (startOver) await startMenu()
            break
        case "linea":
            await lineaFetchDataAndPrintTable().catch(error => { console.error('Виникла помилка:', error)})
            if (startOver) await startMenu()
            break
        case "evm":
            let chain = await evmNetwork()
            await evmFetchDataAndPrintTable(chain).catch(error => { console.error('Виникла помилка:', error)})
            if (startOver) await startMenu()
            break
        case "balances":
